...
# -*- coding: utf-8 -*-
"""
华谊影视 TVBox Python 爬虫源
通过逆向APK获取API协议，实现完整的影视搜索和播放功能

逆向分析:
  - APK: 华谊影视_1.0.0.9.apk (com.muyue.tool)
  - 加壳: Epic Packer (assets/Epic/*.Epic)
  - Spider: csp_AppDrama (spider.jar)
  - 协议: Protobuf + AES加密 + RSA签名
  - API: http://43.240.158.156:8002

JSON端点(无需RSA，已验证可用):
  - 分类列表: GET /api/v3/drama/getCategory?orderBy=type_id
  - 首页推荐: GET /api/ex/v3/security/tag/list
  - 影视列表: GET /api/ex/v3/security/drama/list?typeId1=X&page=Y&pagesize=21
  - 搜索影视: GET /api/ex/v3/security/drama/list?searchKeys=KEYWORD&page=1&pagesize=21
  - 影视详情: GET /api/ex/v3/security/drama/list?vodId=ID&page=1&pagesize=1
  - 播放地址: GET /api/ex/v3/security/videoUsableUrl?vodPlayFrom=SOURCE&playUrl=PATH

Protobuf端点(需要RSA密钥,当publicKey匹配时自动启用):
  - 分类内容: POST /api/proto/v5/drama/category
  - 影视详情: POST /api/proto/v5/drama/getDetail
  - 搜索影视: POST /api/proto/v5/drama/search
  - 播放地址: POST /api/proto/v5/videoUsableUrl
  - 密钥交换: POST /api/v5/find/app/zone
"""
